...
use reth_network::types::{
    EthVersion, GetReceipts, GetReceipts70, NatResolver, PeerKind, ReputationChangeKind,
};
use reth_network::{
...
// A cryptographically verified historical response is stronger evidence than
// a successful handshake. Persist a small positive signal so later runs try
// proven serving peers before arbitrary discovered records.
const VERIFIED_HISTORY_RESPONSE_REPUTATION_REWARD: i32 = 1;
        ...
        loop {
            ...
            let (retry_delay, has_peers) = {
                ...
                let selected = (0..len)
                    ...
                    .filter(|index| {
                        peers[*index].in_flight < per_peer_limit && peers[*index].retry_at <= now
                    })
                    .min_by_key(|index| (peers[*index].failures, peers[*index].in_flight));
                if let Some(index) = selected {
                    peers[index].in_flight = peers[index].in_flight.saturating_add(1);
                    return Ok(DirectPeerLease {
                        pool: self.clone(),
                ...
                (
                    peers
                        .iter()
                        .filter(|peer| peer.in_flight < per_peer_limit)
            ...
            let unavailable_remaining = if has_peers {
                ...
                if elapsed >= unavailable_timeout {
                    return Err(P2pError::Timeout {
                        component: "receipt peer availability",
                    });
                }
...
fn spawn_mainnet_dns_peer_seeder(
    ...
    tokio::spawn(async move {
        ...
        if let Err(error) = control.sync_tree(MAINNET_DNS_DISCOVERY_TREE) {
            warn!(%error, "could not configure mainnet execution peer DNS seeder");
            return;
        }
        ...
        loop {
            tokio::select! {
                ...
                update = records.next() => {
                    ...
                    if dial_record_ids.insert(record.id) {
                        dial_records.push_back(record);
                    }
...
fn reward_verified_history_peer(handle: &NetworkHandle<EthNetworkPrimitives>, peer_id: B512) {
    handle.reputation_change(
        peer_id,
        ReputationChangeKind::Other(VERIFIED_HISTORY_RESPONSE_REPUTATION_REWARD),
    );
}

fn eager_refill_attempts(
    connected: usize,
    preferred: usize,
    maximum_dials: usize,
    records: usize,
            ...
            tokio::select! {
                ...
                event = network_events.next(), if network_events_open => {
                    match event {
                        ...
                        Some(NetworkEvent::Peer(PeerEvent::SessionEstablished(_))) => {
                            network_telemetry.peer_session_established();
                        }
                        Some(NetworkEvent::Peer(PeerEvent::SessionClosed { peer_id, reason })) => {
                            direct_peers.remove(peer_id);
                            let reason = classify_disconnect_reason(reason);
                            network_telemetry.peer_session_closed(reason);
                            debug!(
                                reason = reason.as_str(),
                                "execution peer session closed"
                            );
                        }
                        Some(NetworkEvent::Peer(PeerEvent::PeerAdded(_) | PeerEvent::PeerRemoved(_))) => {}
...
fn write_peer_cache_atomically(
    ...
    if let Err(error) = compact_peer_cache_file(&temporary, maximum_entries) {
        let _ = std::fs::remove_file(&temporary);
        return Err(error);
    }
...
fn compact_existing_peer_cache(path: &Path, maximum_entries: usize) -> Result<(), String> {
    ...
    let compacted = compact_peer_cache_entries(entries, maximum_entries);
...
fn compact_peer_cache_file(path: &Path, maximum_entries: usize) -> Result<(), String> {
...
fn peer_cache_priority(entry: &serde_json::Value) -> (bool, bool, i64) {
    let has_fork = entry
        .get("fork_id")
    ...
    let reputation = entry
        ...
        .unwrap_or_default();
    (reputation >= 0, has_fork, reputation)
}
...
impl RethP2pSource {
    ...
    pub fn mainnet(config: RethP2pConfig) -> Result<Self, P2pError> {
        ...
        if let Some(path) = config.peer_cache_path.as_deref()
            && let Err(error) = compact_existing_peer_cache(path, config.peer_cache_max_entries)
        {
            warn!(
                path = %path.display(),
                %error,
        ...
        let network = Arc::new(PersistentNetwork {
            ...
            direct_peers: Arc::new(DirectPeerPool::default()),
        });
        Ok(Self {
            descriptor: SourceDescriptor {
    ...
    fn peers_config(&self) -> PeersConfig {
        if let Some(path) = self.config.peer_cache_path.as_deref()
            && let Err(error) =
                compact_existing_peer_cache(path, self.config.peer_cache_max_entries)
        {
            warn!(
                path = %path.display(),
                %error,
    ...
    async fn connect(
        ...
        let already_connected = session.fetch.num_connected_peers();
        ...
        let connected = match wait_for_peers(
            &session,
    ...
    async fn fetch_verified_range(
        ...
        let (body_peers, bodies) = match self
        ...
        let (receipt_peers, receipts) = match self
    ...
    async fn poll_next_verified_frame(
        ...
        let permit = self
            .network
            .request_gate
            .acquire(budget.max_in_flight_requests, Priority::High, cancellation)
            .await?;
        self.config
            .network_telemetry
        ...
        let response = match cancellable_peer_request(
            session.fetch.get_headers_with_priority(
                HeadersRequest::one(BlockHashOrNumber::Number(next.0)),
                Priority::High,
            ),
        ...
        let classified = classify_polled_header(next, headers).inspect_err(|_| {
        ...
        let Some(header) = classified else {
...
impl RethP2pHistorySource {
    ...
    async fn run_bridge(
        ...
        let (header_peer, headers) = self
            ...
            .await?;
        reward_verified_history_peer(&session.handle, header_peer);
        let result = self
    ...
    async fn stream_anchored_frames(
        ...
        for header_chunk in headers.chunks(MAX_HISTORY_MATERIAL_WINDOW_BLOCKS) {
            ...
            let mut frames = if sparse_log_scope(request).is_some() {
    ...
    async fn fetch_sparse_log_bodies(
        ...
        let (peers, bodies) = self
            ...
            .await?;
        for peer in peers {
            reward_verified_history_peer(&session.handle, peer);
        }
            ...
            match response {
                Ok(response) => {
                    ...
                    match validate_receipts_against_headers(headers, &response.receipts) {
                        Ok(()) => {
                            lease.succeeded();
                            reward_verified_history_peer(&session.handle, peer_id);
...
async fn wait_for_peers(
    ...
    let mut next_periodic_report = Instant::now();
    loop {
        if !session.manager_is_current().await {
            return Err(P2pError::Network(
                "execution P2P manager restarted after peer discovery stalled".to_owned(),
            ));
        }
        let connected = session.fetch.num_connected_peers();
        if connected >= minimum {
            debug!(
                connected_peers = connected,
                required_peers = minimum,
                "execution peer threshold reached"
            );
            return Ok(connected);
        }
        let now = Instant::now();
        if last_reported != Some(connected) || now >= next_periodic_report {
            debug!(
                connected_peers = connected,
                required_peers = minimum,
                remaining_seconds = deadline.saturating_duration_since(now).as_secs(),
                "waiting for execution peers"
            );
            last_reported = Some(connected);
            next_periodic_report = now + Duration::from_secs(10);
        }
        if now >= deadline {
...
mod tests {
    ...
    #[test]
    fn peer_cache_compaction_is_bounded_and_prefers_verified_metadata() {
        ...
        let compacted = compact_peer_cache_entries(entries, 2);
        assert_eq!(compacted.len(), 2);
        assert_eq!(compacted[0]["record"], "enode://fork");
        assert_eq!(compacted[1]["record"], "enode://plain");
        assert_eq!(compacted[1]["reputation"], 100);
        assert!(
            compacted
                .iter()
    ...
    async fn empty_direct_peer_pool_has_a_distinct_availability_timeout() {
        let pool = Arc::new(DirectPeerPool::default());
        let error = pool
