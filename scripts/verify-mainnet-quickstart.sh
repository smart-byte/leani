...
source = source.replace('127.0.0.1:8080', '127.0.0.1:18180')
source = source.replace('127.0.0.1:8545', '127.0.0.1:18545').replace('127.0.0.1:8546', '127.0.0.1:18546')
...
for _ in {1..60}; do
  ...
  if curl -fsS http://127.0.0.1:18180/health/live > /dev/null 2>&1; then break; fi
  sleep 0.2
done
api_base='http://127.0.0.1:18180/v1/processors/uniswap-v2-sync-30d'
