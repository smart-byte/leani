---
title: Install Leani
description: Build the release candidate, check the binary, and prepare the TypeScript SDK if you need it.
...
Expect `leani 0.1.0-rc.1`. If you already have the checkout, start at
...
Repository examples resolve `@smart-byte/leani-sdk` to the local source. To use the built
...
<Aside type="note" title="Available after publication">
  ...
  artifacts. Use the source instructions above for the current release candidate.
</Aside>
...
SDK with `bun add @smart-byte/leani-sdk@0.1.0-rc.1`. Prerelease SDKs use npm's `next` tag.
